...
func (self *ImportMaster) needReloadBlock(blockNumber int64) bool {
	block := self.GetBlocksByNumber(strconv.Itoa(int(blockNumber)))
	parentBlockNumber := block.Number - 1
	parentBlock := self.GetBlocksByNumber(strconv.Itoa(parentBlockNumber))
	if parentBlock != nil {
		log.Info().Str("ParentHash", block.ParentHash).Str("Hash from parent", parentBlock.BlockHash).Int("BlockNumber", block.Number).Int("ParentNumber", parentBlock.Number).Msg("Checking parent")
	}
...
func (self *ImportMaster) TransactionsConsistent(blockNumber int64) bool {
	...
	if block != nil {
		log.Info().Int("Transactions in block", block.TxCount).Int("Num of transactions in db", 0).Msg("Checking parent")
		iter := self.fs.Collection("Transactions").Where("block_number", "==", blockNumber).Documents(self.ctx)
		for {
			_, err := iter.Next()
			...
			if err != nil {
				log.Fatal().Err(err).Msg("TransactionsConcistent")
			}
			transactionCounter++
		}
