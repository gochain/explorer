...
module.exports = function (req, res) {
  ...
  else if (req.body.action == "dbstats")
    getDbStats(res);
...
**/
var getDbStats = function (res) {
  Transaction.count({}, function (err, transactionResult) {
    if (err) {
      console.error(err);
      res.status(500).send();
    } else {
      Block.count({}, function (err, blockResult) {
        if (err) {
          console.error(err);
          res.status(500).send();
        } else {
          console.log("Number of blocsk in DB:",blockResult);
          res.write(JSON.stringify(
            {
              transactionsDb: transactionResult,
              blocksDb: blockResult
            }
          ));
          res.end();
        }
